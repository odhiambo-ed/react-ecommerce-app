...
function Items() {
    ...
    useEffect(() => {
        if (data.length !== 0) {
            let calculated = data.map((val) => val.data.cost);
            let sum = calculated.reduce((a, b) => a + b);
            setTotal(sum);
            let sets = data.map((val) => val.data.quantity);
            let setss = sets.reduce((a, b) => a + b);
                                    ...
                                    <div className="col-lg-4 bg-grey">
                                        <div className="p-5">
                                            ...
                                            <div className="d-flex justify-content-between mb-4">
                                                <h5 className="text-uppercase">items {totalLength}</h5>
                                                <h5>Ksh. {total}</h5>
                                            </div>
                                            <hr className="my-4" />
                                            {/* paypal */}
